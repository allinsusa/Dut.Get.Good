...
namespace Dut.Get.Good.GetGoodRepository.ClassAbiltiesRepository
{
    public class CallAbilitiesRepository : EfCoreRepository<GoodDbContext, ClassAbility, Guid>, ICallAbilitiesRepository
    {
        ...
        public async Task<List<ClassAbilitiesFullDetails>> GetAllClassAbilities(CancellationToken cancellationToken = default)
        {
            await Configuration.CommandAndConnectionManager.RepositoryCommandAndConnectionManager.EnsureConnectionOpenAsync(await GetDbContextAsync(), cancellationToken);
            ...
            return await dataReader.MapToList<ClassAbilitiesFullDetails>(cancellationToken);
        }

        public async Task<ClassAbilitiesFullDetails> GetAttributeById(Guid ClassAbilityId, CancellationToken cancellationToken = default)
        {
            var parameters = new SqlParameter[]
             {
            ...
            using var dataReader = await command.ExecuteReaderAsync(cancellationToken);
            return await dataReader.MapToObject<ClassAbilitiesFullDetails>(cancellationToken);
        }
        public async Task<List<ClassAbilitiesFullDetails>> GetAllClassAbilitiesByClassId(Guid ClassId, CancellationToken cancellationToken = default)
        {
            var parameters = new SqlParameter[]
             {
